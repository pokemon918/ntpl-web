import React from 'react';
import {action} from '@storybook/addon-actions';

import {IWineResponse} from 'types/wines';
import Grid from '../Grid';
import WineList from '../WineList';
import WineFilter from './index';

export default {
	title: 'UI Kit / Inputs / Wine Filter',
	component: WineFilter,
};

const winesListItems = [
	{
		ref: 111,
		name: 'Niederhauser Felsensteyer Riesling Spatlese',
		producer: 'Crusius',
		vintage: '2008',
		region: 'Nahe',
		country: 'Germany',
		created_at: {date: '2019-03-21'},
		rating: {final_points: 97},
	},
	{
		ref: 222,
		name: 'Eitelsbacher Karthauserhofberg Riesling Trockenbeerenauslese Auktion Nr20',
		producer: 'Karthauserhof',
		vintage: '1999',
		region: 'Mosel',
		country: 'Germany',
		location: 'Hjemme hos Karsten',
		created_at: {date: '2019-03-21'},
		rating: {final_points: 85},
	},
	{
		ref: 333,
		name: 'Palacio de Anglona Tempranillo',
		producer: 'Hombre',
		vintage: '2008',
		region: 'Rioja',
		country: 'Spain',
		location: 'Avedøre, Denmark',
		created_at: {date: '2019-03-21'},
		rating: {final_points: 69},
	},
];

export const normal = () => (
	<Grid columns={5}>
		<WineFilter wines={winesListItems}>
			{({filteredWines}: {filteredWines: IWineResponse[]}) => (
				<WineList wines={filteredWines} onClickWine={action('Clicked')} />
			)}
		</WineFilter>
	</Grid>
);
